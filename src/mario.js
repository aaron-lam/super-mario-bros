class Mario {
  ...
  loadAnimations() {
    for (let i = 0; i < 6; i += 1) { // six states
      this.animations.push([]);
      for (let j = 0; j < 3; j += 1) { // three sizes (star-power not implemented yet)
        this.animations[i].push([]);
        for (let k = 0; k < 2; k += 1) { // two directions
        ...
        if (entity.BB && that.BB.collide(entity.BB)) {
          ...
          if ((entity instanceof Tube || entity instanceof Block || entity instanceof Ground) && that.BB.bottom > entity.BB.top) {
            ...
            if (!that.game.right && !that.game.left) {
              ...
              that.velocity.x = -(1 ** that.facing) * MIN_WALK;
            }
            that.updateBB();
          }

class SceneManager {
  ...
  update() {
    ...
    const midpoint = PARAMS.CANVAS_WIDTH / 2 - PARAMS.BLOCKWIDTH / 2;
  ...
  draw(ctx) {
    ...
    if (PARAMS.DEBUG) {
      const xV = `xV=${Math.floor(this.game.mario.velocity.x)}`;
      const yV = `yV=${Math.floor(this.game.mario.velocity.y)}`;
